namespace DataStore.Tests.Tests.Partitions.Read
{
    ...
    public class WhenCallingReadWithSyntheticKeys
    {
        ...
        private async void AndAttributeWithTypeIdKeyItShouldReturnTheCorrectCount()
        {
            ...
            await Assert.ThrowsAsync<CircuitException>(async () =>  await testHarness.DataStore.Read<AggregateWithTypeIdKey>(null, options => options.ProvidePartitionKeyValues(Guid.NewGuid())));
        ...
        private async void AndAttributeWithTypeTenantIdKeyItShouldNotRequireFullKey() //because you dont know last part which is ID
        {
            ...
            await Assert.ThrowsAsync<CircuitException>(async () => await testHarness.DataStore.Read<AggregateWithTypeTenantIdKey>(null, options => 
                              options.ProvidePartitionKeyValues(MonthInterval.FromDateTime(DateTime.UtcNow)))); //throw for irrelevant option

            await Assert.ThrowsAsync<CircuitException>(async () => await testHarness.DataStore.Read<AggregateWithTypeTenantIdKey>(null, options => 
                          options.ProvidePartitionKeyValues(Guid.NewGuid()))); //throw for useless options
        ...
        {//requires full key or nothing because its possible to obtain it and there is no fallback in synth mode
            ...
            await Assert.ThrowsAsync<CircuitException>(async () => await testHarness.DataStore.Read<AggregateWithTypeTenantTimePeriodKey>(
                              null,
            ...
            results = await testHarness.DataStore.Read<AggregateWithTypeTenantTimePeriodKey>(
                          null,
                          options => options.ProvidePartitionKeyValues(tenantId, HourInterval.FromDateTime(DateTime.UnixEpoch)));
            Assert.Empty(results);  //without wrong key  fails to return anything
        ...
        private async void AndAttributeWithTypeTimePeriodKeyItShouldNotRequireTheFullKey()
        {
            ...
            await Assert.ThrowsAsync<CircuitException>(async () => await testHarness.DataStore.Read<AggregateWithTypeTimePeriodIdKey>(null, options => 
                          options.ProvidePartitionKeyValues(DayInterval.FromDateTime(DateTime.UtcNow)))); //throw for useless option
            
            
            await Assert.ThrowsAsync<CircuitException>( async () => await testHarness.DataStore.Read<AggregateWithTypeTimePeriodIdKey>(null, options => 
                          options.ProvidePartitionKeyValues(Guid.NewGuid()))); //throw when sending wrong type of option
