namespace DataStore.Tests.Tests.Partitions.Read
{
    ...
    public class WhenCallingReadByIdWithSyntheticKeys
    {
        [Fact]
        private async void AndAttributeWithSharedKeyItShouldReturnTheRecord()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingReadByIdWithSyntheticKeys) + nameof(AndAttributeWithSharedKeyItShouldReturnTheRecord),
            ...
            //when
            var result = await testHarness.DataStore.ReadById<AggregateWithSharedKey>(newId);
            Assert.NotNull(result);
            
            result = await testHarness.DataStore.ReadById<AggregateWithSharedKey>(Guid.NewGuid());
            Assert.Null(result);
        }
        ...
        private async void AndAttributeWithTypeIdKeyItShouldReturnTheRecord()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingReadByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeIdKeyItShouldReturnTheRecord),
                useHierarchicalPartitionKey: false);
        ...
        private async void AndAttributeWithTypeTenantIdKeyItShouldReturnTheRecordOnlyWithFullKey()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingReadByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTenantIdKeyItShouldReturnTheRecordOnlyWithFullKey),
                useHierarchicalPartitionKey: false);
        ...
        private async void AndAttributeWithTypeTenantTimePeriodKeyItShouldReturnTheRecordOnlyWithTheFullKey()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingReadByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTenantTimePeriodKeyItShouldReturnTheRecordOnlyWithTheFullKey),
                useHierarchicalPartitionKey: false);
        ...
        private async void AndAttributeWithTypeTimePeriodKeyItShouldReturnTheRecordOnlyWithFullKey()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingReadByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTimePeriodKeyItShouldReturnTheRecordOnlyWithFullKey),
                useHierarchicalPartitionKey: false);
