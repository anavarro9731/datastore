namespace DataStore.Tests.Tests.Partitions.Update
{
    ...
    public class WhenCallingUpdateByIdWithSyntheticKeys
    {
        ...
        private async void AndAttributeWithSharedKeyItShouldUpdateIt()
        {
            ...
            var testHarness = TestHarness.Create(
                nameof(WhenCallingUpdateByIdWithSyntheticKeys) + nameof(AndAttributeWithSharedKeyItShouldUpdateIt),
            ...
            //when
            var result = await testHarness.DataStore.UpdateById<AggregateWithSharedKey>(newId, a => a.TestValue = 1);
        ...
        private async void AndAttributeWithTypeIdKeyItShouldUpdateIt()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingUpdateByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeIdKeyItShouldUpdateIt),
                useHierarchicalPartitionKey: false);
            ...
            //when
            var result = await testHarness.DataStore.UpdateById<AggregateWithTypeIdKey>(newId, a => a.TestValue = 1);
            Assert.NotNull(result);
        ...
        private async void AndAttributeWithTypeTenantIdKeyItShouldUpdateIt()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingUpdateByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTenantIdKeyItShouldUpdateIt),
                useHierarchicalPartitionKey: false);
            ...
            //when
            var result = await testHarness.DataStore.UpdateById<AggregateWithTypeTenantIdKey>(newId, a => a.TestValue = 1, options => options.ProvidePartitionKeyValues(tenantId));
            Assert.NotNull(result);
        ...
        private async void AndAttributeWithTypeTimePeriodKeyItShouldUpdateIt()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingUpdateByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTimePeriodKeyItShouldUpdateIt),
                useHierarchicalPartitionKey: false);
            ...
            //when
            var result = await testHarness.DataStore.UpdateById<AggregateWithTypeTimePeriodIdKey>(newId, a => a.TestValue = 1, options => options.ProvidePartitionKeyValues(dayInterval));
            Assert.NotNull(result);
        ...
        private async void AndAttributeWithTypeTenantTimePeriodKeyItShouldUpdateIt()
        {
            // Given
            var testHarness = TestHarness.Create(
                nameof(WhenCallingUpdateByIdWithSyntheticKeys) + nameof(AndAttributeWithTypeTenantTimePeriodKeyItShouldUpdateIt),
                useHierarchicalPartitionKey: false);
            ...
            //when
            var result = await testHarness.DataStore.UpdateById<AggregateWithTypeTenantTimePeriodKey>(newId, a => a.TestValue = 1, options => options.ProvidePartitionKeyValues(tenantId, monthInterval));
            Assert.NotNull(result);
