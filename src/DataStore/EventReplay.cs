namespace DataStore
{
    ...
    public class EventReplay
    {
            ...
            Func<T, bool> predicate) where T : class, IAggregate, new()
        {
            switch (previousUncommittedOperation)
            {
                ...
                case QueuedUpdateOperation<T> _:
                    ...
                    if (!predicate(previousUncommittedOperation.NewModel)
                        && results.Exists(i => i.id == previousUncommittedOperation.NewModel.id))
                    {
        ...
        public List<T> ApplyAggregateEvents<T>(IEnumerable<T> results, Func<T, bool> predicate) where T : class, IAggregate, new()
        {
            ...
            var uncommittedEvents = this.messageAggregator.AllMessages.OfType<IQueuedDataStoreWriteOperation<T>>().OrderBy(e => e.Created)
                                        .Where(e => !e.Committed);
